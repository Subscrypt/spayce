...
const UserCreate: NextPage<IProps> = ({ user }) => {
  ...
  const onSubmit = async (values: IFormValues) => {
    try {
      ...
      replace("/users");
      queryClient.setQueryData<InfiniteData<TPaginationResult<User>> | undefined>("users", data => {
        const page = data?.pages.find(page => page.data.some(userElem => userElem.id === user.id));
        if (page) {
  ...
  return (
    <Layout title={user.username} backRoute="/users">
      <VStack spacing={4} width="100%">
        ...
        <UserForm initialValues={{ username: user.username }} onSubmit={onSubmit} />
      </VStack>
    </Layout>
  );
};
...
export const getServerSideProps: GetServerSideProps<IProps> = async ctx => {
  const user = await fetch(`${process.env.API_URL}/users/${ctx.query.id}`).then(res => res.json());
