                ...
                <div className='flex flex-col gap-6'>
                    <div className='flex flex-col'>
                        ...
                        <div className='grid grid-cols-2 gap-1 justify-between p-1 bg-gray-100 rounded-xl'>
                            {subs?.members.map((elem, key) => <><FullSubscriber key={key} user={(elem.user.address === "" && elem?.user?.name === 'Empty slot') ? elem.user.name : elem.user.address} /></>)}
                        </div>
                    </div>
                    <div className='flex flex-col mb-6'>
                        ...
                        <div className='grid grid-cols-1 border-2 border-gray-100 h-32 overflow-scroll gap-1 justify-between p-1 bg-gray-100 rounded-xl'>
                            {subs ? subs.payments.map((el, ke) =>
                                <SubsBilling key={ke} user={el.member?.user?.name ? el.member.user.name : el.user?.address as string} date={el.createdAt.toString().slice(0, 10)} price={el.amount} />)
                                : ''}
                        </div>
                    </div>
                </div>
